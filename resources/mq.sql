...
DROP DATABASE IF EXISTS mq CASCADE;
...
-- zone configs
alter partition mq_0 of table mq configure zone using 
    num_replicas = 3, constraints = '{+zone=a: 1}', lease_preferences = '[ [+zone=a] ]';
alter partition mq_1 of table mq configure zone using 
    num_replicas = 3, constraints = '{+zone=b: 1}', lease_preferences = '[ [+zone=b] ]';
alter partition mq_2 of table mq configure zone using 
    num_replicas = 3, constraints = '{+zone=d: 1}', lease_preferences = '[ [+zone=d] ]';
...
ALTER TABLE hwm PARTITION BY RANGE (bucket) (
    PARTITION hwm_0 VALUES FROM (0)   TO (86),
    PARTITION hwm_1 VALUES FROM (86)  TO (173),
    PARTITION hwm_2 VALUES FROM (173) TO (256)
);

-- zone configs
alter partition hwm_0 of table hwm configure zone using 
    num_replicas = 3, constraints = '{+zone=a: 1}', lease_preferences = '[ [+zone=a] ]';
alter partition hwm_1 of table hwm configure zone using 
    num_replicas = 3, constraints = '{+zone=b: 1}', lease_preferences = '[ [+zone=b] ]';
alter partition hwm_2 of table hwm configure zone using 
    num_replicas = 3, constraints = '{+zone=d: 1}', lease_preferences = '[ [+zone=d] ]';
